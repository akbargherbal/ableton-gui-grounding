...
**A diff-level fix plan for these four was proposed in-conversation this session (not yet in a file) and is NOT YET APPROVED OR APPLIED.** The instrumentation gap (item 5 below) was deliberately left out of that plan pending a bundling decision. No code has been touched this session — `orchestrate.sh` and `automate_ableton_task.py` in this working copy are still byte-identical to GitHub.

**Still open, per standing instruction not to decide unilaterally — now to be revisited jointly next session in light of Section 6's limitations, not decided in isolation:**
- Whether to add a "report bugs, don't patch them unprompted" rule to `ABLETON_AGENT_POLICY.md` (scope creep confirmed in 2 of 7 analyzed transcripts: `result_01.md`/P0.1, `result_05.md`/P1.3).
...
- Whether the screenshot-instrumentation gap (`set_tempo`/`read_solo_states` lacking per-substep `EVENT:` emission, unlike `arm_track`/`solo_one`) belongs in the same fix pass as the four code bugs or a separate one.

**Next step (session 12): joint review of `docs/synthesis_session11.md` — including Section 6 — before committing to the fix pass.** The user is skimming the synthesis now; next session both the user and the AI Assistant will go through it thoroughly together and decide which path to take, informed by which findings are genuinely fixable (Sections 1–4) versus which are hard architectural/environmental ceilings (Section 6) that a fix pass shouldn't be scoped around solving. Only after that decision does the sequence originally planned for session 11 resume: apply the agreed fix scope, re-run `build_runtime_env.sh` to refresh the runtime copy, confirm fixes present, run the existing test suite, spot-check bug #4 live from a non-120 starting tempo (never yet proven live), then update this file to "Comprehensive fix: DONE" before Tiers 3–7 (P3.1–P7.1) begin — these need fresh live probe sessions against a running Ableton instance, one probe per session per the established cold-start discipline; no transcripts exist yet for these tiers.