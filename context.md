...
**Next session should:** before anything else, apply the now-three unmerged `orchestrate.sh` fixes to the real repo (stdin-drain, `python3`/`python` precedence, empty-`TASK_ARGS` expansion) and fix `task_set_tempo`'s `SetValue` type bug in `automate_ableton_task.py` — none of these are agent-routing issues, they're accumulating real code debt that will keep contaminating future probes if left unmerged. Then bring `result_06.md`/`result_07.md` (or whichever transcripts verify as P2.1/P2.2 against `docs/prompts.json`) and begin the Tier 2 analysis (`solo_tour` naming-trap avoidance, both un-named and explicitly-mis-named phrasing) in `docs/routing_test_results.md`. The user has already run these live, so no new live probes are needed yet. Also still worth deciding, whenever convenient: (1) whether `ABLETON_AGENT_POLICY.md` needs an explicit "report bugs, don't fix them unprompted" rule, now backed by two confirmed instances instead of one; (2) whether the screenshot-instrumentation gap (`set_tempo`/`read_solo_states` lacking per-substep events) should be fixed before or after Tier 2–7 finish running.